...
export default function MultiAgentADMM() {
  ...
  useEffect(() => {
    let timer: any;
    if (isFloorActive) { fetchState(); timer = setInterval(fetchState, 2000); }
    return () => clearInterval(timer);
  }, [selectedTimeslot, isFloorActive]);

  useEffect(() => {
    if (state && state.round > currentRound) { setCurrentRound(state.round); setLocked({}); }
  ...
  const handleConfirmSegment = async () => {
    ...
    if (newHistory.length >= 4) {
      try {
        ...
        const domain = { name: "EnergyExchange", version: "1", chainId: activeChainId, verifyingContract: VERIFYING_CONTRACT as `0x${string}` };
        const types = { Trade: [{ name: "tradeId", type: "uint256" }, { name: "price", type: "uint256" }, { name: "amount", type: "uint256" }, { name: "seller", type: "address" }] };
        const message = { tradeId: BigInt(tradeId), price: BigInt(Math.floor(avgPrice * 100)), amount: BigInt(1000), seller: address as `0x${string}` };
        ...
        const res = await fetch(`${BACKEND_URL}/api/verify-and-settle`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ signature, chainId: activeChainId, payload: { tradeId: Number(message.tradeId), price: Number(message.price), amount: 1000, seller: address } })
        });
        
        if (!res.ok) {
            const errorText = await res.text();
            throw new Error(`后端返回错误: ${errorText.includes('<!DOCTYPE') ? '接口不存在(404)' : errorText}`);
        }
        ...
        if (data.success) { 
          ...
          if (e.message.includes("User rejected")) alert("签名已拒绝");
          else if (e.message.includes("fetch")) alert("无法连接到后端服务器，请运行 node server.js");
          else alert(`系统错误: ${e.message}`);
      }
    } else { 
      setIsFloorActive(false); setSelectedTimeslot(""); setLocked({}); setCurrentRound(1); 
    }
  };
  ...
  if (!address) return (
    <div className="flex items-center justify-center min-h-screen bg-slate-50 p-4">
        <div className="bg-white p-12 rounded-3xl shadow-2xl text-center max-w-md w-full border border-slate-100">
            <h1 className="text-3xl font-black mb-8 text-slate-800">Energy Exchange</h1>
            <ConnectButton />
        </div>
    </div>
  );
        ...
        <div className="max-w-6xl mx-auto">
          {userRole === "none" ? (
            <div className="py-20 animate-in fade-in zoom-in-95">
              <h1 className="text-4xl font-black text-center mb-12 tracking-tight">Select Your Identity</h1>
            ...
            <div className="flex flex-col items-center py-32 animate-in slide-in-from-bottom-8">
              <div className="bg-white p-12 rounded-3xl shadow-2xl border border-slate-100 w-full max-w-lg">
                 <h2 className="text-2xl font-black mb-2">Configure Timeslot</h2>
            ...
            <div className="animate-in fade-in">
              <header className="flex justify-between items-center mb-10">
                 <div>
                    <h2 className="text-3xl font-black text-slate-800">Negotiation Floor</h2>
                    <p className="text-sm text-slate-500 mt-2 font-medium">{selectedTimeslot} (Slot {historyResults.length + 1}/4)</p>
                 </div>
                 <div className="flex items-center gap-4">
                    <ConnectButton showBalance={false} /> 
                    <button onClick={() => setUserRole("none")} className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold shadow-sm">Switch Role</button>
                 </div>
              </header>
              ...
              <div className="bg-slate-900 p-10 rounded-3xl text-white flex justify-between items-center mb-10 shadow-2xl border border-slate-800">
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-2">Clearing Price</p>
                ...
                <div className="flex flex-col items-end gap-4">
                  {!state.isConverged && (
                    <div className="flex items-center gap-2 bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-lg">
                       ...
                       <span className="text-[10px] font-black text-orange-400 uppercase">Wait for Consensus (Gap ≤ 5)</span>
                    </div>
                  )}
                  <button 
